...
export function detectPattern(filenames) {
    ...
    for (let p = 0; p < majorityLength; p++) {
        if (!stable[p] && dateTokenIndex === -1) {
            
            if (isTokenDate(majorityTokens[p])) {
                
                let dateCount = 0;
                for (const fileParts of parts) {
                    if (isTokenDate(fileParts[p])) {
                        dateCount++;
                    }
                }
                ...
                if (dateCount / fileCount >= MAJORITY_THRESHOLD) {
                    dateTokenIndex = p;
                    majorityDateToken = majorityTokens[p];
                    break;
                }
            }
        }
    ...
    for (const fileParts of parts) {
        ...
        if (fileParts.length !== majorityLength) {
            ...
            for (let p = 0; p < majorityLength; p++) {
                if (p !== dateTokenIndex && stable[p] && fileParts[p] !== majorityTokens[p]) {
                    isMinority = true;
                    break;
                }
            }
    ...
    const variableIndexes = stable
        .map((s, i) => (!s && i !== dateTokenIndex) ? i : null)
        .filter(v => v !== null);
    ...
    return {
        ...
        dateTokenIndex: dateTokenIndex,
        majorityDateToken: majorityDateToken
    };
}