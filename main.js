...
btnSelectFolder.addEventListener("click", async () => {
    if (ipcRenderer && typeof ipcRenderer.invoke === 'function') {
        ...
        if (!folderInput) return alert('Folder input not found');
...
function analyzePattern() {
    try {
        ...
        detectedPattern = null;
        detectedSegmentsContainer.innerHTML = `<div class="error">Pattern detection failed: ${err.message || err}</div>`;
    }
}
...
function renderDetectedSegments() {
    try {
        ...
        const dateIndexes = detectedPattern.dateTokenIndexes || [];
        ...
        if (!detectedPattern.isUniform) {
            ...
            warning.innerHTML =
                `⚠️ <strong>Inconsistent Pattern Detected</strong> <br>` +
        ...
        if (!Array.isArray(exampleTokens) || exampleTokens.length === 0) {
            detectedSegmentsContainer.innerHTML = "<p>No tokens detected</p>";
            return;
        }
        ...
        exampleTokens.forEach((tok, idx) => {
            ...
            const datePos = dateIndexes.indexOf(idx);
            if (datePos !== -1) {
                label = `@date${datePos + 1}`;
        ...
        console.error('Render detected segments error:', err);
        detectedSegmentsContainer.innerHTML = `<div class="error">Failed to render detected segments: ${err.message || err}</div>`;
    }
}
...
function applyCustomTemplate(files, pattern, parts) {
    ...
    const dateIndexes = pattern.dateTokenIndexes || [];
    const majorityDateTokens = pattern.majorityDateTokens || {};

    const tentative = files.map((file) => {
        const tokens = splitNameTokens(file.name, pattern.delimiter);

        let newBaseName = parts.map(p => {
            const paramMatch = p.match(/^@param(\d+)$/i);
            if (paramMatch) {
                const idx = parseInt(paramMatch[1], 10) - 1;
                return tokens[idx] ?? "";
            }

            const dateMatch = p.match(/^@date(\d*)$/i);
            if (dateMatch) {
                const n = dateMatch[1] ? parseInt(dateMatch[1], 10) : 1;
                const datePos = dateIndexes[n - 1];
                let fullToken = (datePos !== undefined) ? (tokens[datePos] ?? majorityDateTokens[datePos] ?? "") : "";

                if (fullToken) {
                    const shortMatch = fullToken.match(/^(\d{8})/);
            ...
            const yearMatch = p.match(/^@year(\d*)$/i);
            if (yearMatch) {
                const n = yearMatch[1] ? parseInt(yearMatch[1], 10) : 1;
                const datePos = dateIndexes[n - 1];
                const fullToken = (datePos !== undefined) ? (tokens[datePos] ?? majorityDateTokens[datePos] ?? "") : "";
                const parsed = parseDateFromName(fullToken || "");
                return parsed ? parsed.year : "";
            }

            const monthMatch = p.match(/^@month(\d*)$/i);
            if (monthMatch) {
            ...
            const dayMatch = p.match(/^@day(\d*)$/i);
            if (dayMatch) {
    ...
    const used = new Set();
    const finalResults = tentative.map(item => {
...
async function applyRenameAction() {
    ...
    if (ipcRenderer && typeof ipcRenderer.invoke === 'function' && currentFiles[0] && currentFiles[0].path) {
